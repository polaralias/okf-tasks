---
type: Task
task: exercise-rollback-automation
title: Exercise rollback automation
description: Deliver exercise rollback automation for the Observability Release initiative.
status: ready
created: '2026-07-11T08:00:00Z'
timestamp: '2026-07-11T12:00:00Z'
owner: team-observability-release
priority: normal
navigation:
  role: supporting
  order: 240
tags:
- observability-release
- complex-example
- wave-3
depends_on:
- add-release-health-checks/task.md
- create-cross-service-trace-model/task.md
---

# Exercise rollback automation

## Outcome

Deliver the observable result described by **exercise rollback automation** and leave reviewable evidence for the next task.

## Scope

- In scope: implementation, validation, and handoff for this delivery slice.
- Out of scope: unrelated product changes and production credentials.

## Acceptance

- [ ] The result is demonstrable in the dummy programme environment.
- [ ] Relevant tests and operational evidence are linked.
- [ ] Follow-on tasks can consume the outcome without rediscovery.

## Dependencies and risks

- [Add Release Health Checks](../add-release-health-checks/task.md)
- [Create Cross Service Trace Model](../create-cross-service-trace-model/task.md)

## Related knowledge

- [Programme guidance](../../docs/observability-release.md)

## Evidence

- Dummy evidence will be attached as the task advances through validation.
