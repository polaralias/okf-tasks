...
The viewer defaults to a Focus relationship explorer in Graph, with the selected record between readable incoming and outgoing record cards. Topology mode keeps the complete bundle available with bounded compact labels, class-colored borders, switchable layouts, and relationship labels around the active node. Kanban groups Tasks and Workstreams by lifecycle status, both graph modes share one detailed preview, and Documents provides a searchable near-full-page Markdown reader. The viewer surfaces effort and connection metrics, summarizes time by workstream, and can arrange or filter current records by `timestamp`, `created`, `started`, or `finished`. Its drift review compares timestamps across existing links and carries possible signals into Graph and Kanban. Report every highlight as a possible review signal, never proof that an older target is stale; the current bundle does not reconstruct historical record bodies.
