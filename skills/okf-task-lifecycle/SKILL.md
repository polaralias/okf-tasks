...
description: Create and maintain OKF Tasks bundles containing portable repository-local task records, workstreams, lifecycle transitions, embedded time entries, effort evidence, knowledge links, definitive Graph, Board, and Reader visual review, first-class GitHub, GitLab, Linear, and ClickUp Tracker Profiles, safe external artifacts, and generated indexes. Use when work must remain durable beside a repository, survive beyond chat or a tracker, or be exchanged as Markdown with YAML frontmatter without leaking secrets or machine-local paths.
