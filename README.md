...
The maintained review set includes three generated stress-test workspaces. `okf-tasks-complex-task-portfolio.html` contains [forty linked Tasks, five coordinating Workstreams, mixed lifecycle states, and embedded time evidence](./examples/complex-task-portfolio/README.md). `okf-tasks-architecture-knowledge-base.html` contains [twenty ADRs plus detailed architecture, service, interface, quality-attribute, and implementation records](./examples/architecture-knowledge-base/README.md). The combined example [joins both surfaces through an explicit programme map](./examples/combined-delivery-architecture/README.md) so delivery and architecture can be reviewed as one connected graph. Its [interactive workspace](./examples/visualization/combined-workspace.html) and [Mermaid report](./docs/COMBINED_WORKSPACE_VISUALIZATION.md) are tracked, reviewable native exports rather than ignored local-only output. The Markdown/YAML source for every example remains tracked under its example directory; regenerate it through `generate_complex_examples.py` rather than hand-editing generated records.
...
The visualiser includes every Markdown document in Reader and promotes any document with OKF frontmatter into the graph. A durable `README.md` used for project orientation should therefore be a typed, timestamped knowledge concept with `navigation.role: entry-point` and useful relative links; generated, vendored, or transient READMEs do not need promotion. To omit intentionally out-of-scope files from both Graph and Reader, add repeatable `--exclude "path/**"` selections or store one bundle-relative path or glob per line in `<bundle>/.okf-visualization-ignore`. A directory entry such as `node_modules/` matches that directory at any depth; recursive globs such as `**/.venv/**` and `**/.pytest_cache/**` cover both root and nested directories. Use `--exclude-from <file>` for a named policy. The generated payload records both the policy and every omitted Markdown path, while validation still evaluates the complete governed knowledge graph.

Open the HTML file locally as the definitive three-view OKF workspace. Graph presents the complete document relationship mesh and reflows a selected document's incoming and outgoing neighbourhood into a close, readable circular focus while keeping every direct node visible. Its type key and reading-role selector highlight chosen classes while retaining graph context; ADRs, entry points, foundational records, and highly connected landmarks remain labelled in dense overviews, while ordinary low-connectivity records become class-coloured marks until hover or zoom reveals them. Disconnected documents remain available as a compact reveal-on-hover shelf beside the connected map, so they do not force the primary structure to open at an unreadable scale. Its right panel turns that neighbourhood into a compact vertical Incoming → Selected → Outgoing focus view with clickable relationship cards, temporal and effort context, and a Reader shortcut instead of a full document rendering. Board provides lifecycle columns or compact rows for Tasks, with nested Workstreams, embedded effort evidence, estimates, tracker context, and exact temporal values. Reader provides a searchable repository tree, full Markdown document surface, contextual ancestry, connections, and heading navigation. Light mode is the default, dark mode persists locally, and timestamp comparison can flag possible drift across existing relationships without claiming that older content is stale. Embedded `Task.time[]` entries contribute evidence and effort to their Task. Small graphs use compact layout bounds and always fit the complete node set into the initial viewport. The generated file embeds pinned Cytoscape, Marked, DOMPurify, and Mermaid builds plus its sanitised task and document payload, so opening it locally makes no network requests. On Windows, regenerating an HTML output also removes any inherited download-zone marker from that output alone, preventing a stale email or browser-download warning.
